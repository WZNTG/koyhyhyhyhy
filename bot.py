...
async def handle_all_messages(msg: types.Message):
    ...
    if text == "чайхана":
        ...
        # Проверка времени (8 часов = 28800 секунд)
        # Если хочешь еще быстрее, замени 28800 на нужное число секунд
        WAIT_TIME = 28800 

        if user and now - user[1] < WAIT_TIME:
            time_left = WAIT_TIME - (now - user[1])
        ...
        res_text = (
            ...
            f"Следующая попытка через 8 часов!"
        )
